...
Nota: se denomina grafo $K_{m,n}$ a un grafo bipartito completo con $m$ nodos en un conjunto y $n$ nodos en el otro. Es decir, un grafo bipartito en el que todos los nodos de un conjunto están conectados con todos los nodos del otro conjunto.
...
Por lo tanto, $F_{ij}^{n+1} = \sum_{v} A_{kj}F^n_{ik} = \sum_{v} A_{kj}A^n_{ik} = A^{n+1}_{ij}$.  
...
Se dice que dos grafos son **isomorfos** si podemos renombrar los vértices de uno para que sea igual al otro; es decir, si son el mismo grafo pero con visualización y nombres diferentes.
