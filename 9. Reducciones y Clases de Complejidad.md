...
A partir de ahora vamos a plantear los problemas de una forma booleana: por ejemplo, en Independent Set el problema es encontrar el set independiente más grande, pero vamos a plantearlo como "¿Existe un set independiente de tamaño $k$?".
...
<details>
    ...
    2. Creamos un grafo G' que es el complemento de G: para todo par de nodos en G que no están conectados, los conectamos en G', y si están conectados, no los conectamos.
...
4. **¿Qué implica que hayamos reducido este problema a TSP?** Ya vimos que TSP es un problema NP-Completo. Reducimos un problema en NP a uno NP-Completo, la definición de NP-Completo establece que todos los problemas en NP se pueden reducir a él.
...
Pareciera que Independent Set es una buena opción dado que utilizan ambos un grafo y se basan en la misma idea de no tener vértices adyacentes. Sin embargo, no es sencillo buscar una reducción de Independent Set a 3-Coloring, ya que Independent Set consiste en buscar un conjunto independiente de cierto tamaño, y 3-Coloring consiste en buscar 3 conjuntos independientes de cualquier tamaño.
...
![3-Coloring inicial](imagenes/reducciones/3sat_a_3coloreo_1.png)

Ahora vamos a incorporar las expresiones. Podemos representar el OR en nuestro grafo de la siguiente manera:

![OR Gadget](imagenes/reducciones/3sat_a_3coloreo_2.png)
...
![3-Coloring final](imagenes/reducciones/3sat_a_3coloreo_3.png)

De esta forma, podemos forzar que el nodo que representa la cláusula entera sea True.

Volviendo a lo que les decía, no es necesario conectar el nodo $a \lor b$ con Base ya que si $a \lor b$ es Base, necesariamente $a$ o $b$ debe ser True y el otro False, asignándose a $a'$ y $b'$ False y True respectivamente; ya que $a'$, $b'$ y $a \lor b$ forman un triángulo. Es trivial ver que si $a \lor b$ es Base, existe forma de asignar colores de forma que $a \lor b \lor c$ sea True: asignamos False a $(a \lor b)'$ y Base a $c'$ indistintamente del valor de $c$.
...
Dado un conjunto de $n$ elementos y un valor $V$, ¿existe un subconjunto cuya suma sea igual a $V$?

Habíamos visto en Programación Dinámica que es posible resolverlo en tiempo pseudopolinomial: $O(nV)$ = $O(nb \cdot 2^m)$, donde $b$ es la cantidad de bits necesarios para representar cada elemento y $m$ es la cantidad de bits necesarios para representar $V$.
. Es decir, no depende del tamaño de la entrada, sino de los valores de los elementos (por lo que termina siendo exponencial, salvo que $W$ sea un valor polinomialmente dependiente de n).
...
4. Si el problema de scheduling puede resolverse con latencia máxima 0, es porque se pudieron ubicar tareas que suman exactamente W en el intervalo [0, W]. Entonces, la n-ésima tarea se puede ubicar en el intervalo [W, W + 1] y la latencia máxima se mantiene en 0 ubicándose el resto de los elementos en el intervalo [W+1, S+1].  
En el caso de que no se haya podido ubicar tareas que suman exactamente W en el intervalo [0, W], necesariamente ocurre alguno de los siguientes casos:
    - Se agrega tiempo muerto antes de la tarea extra ubicada en el intervalo [W, W + 1] para no generar latencia de ella. Como en este caso el tiempo utilizado total sería mayor a $S+1$, la latencia máxima sería mayor a 0 dado que la última tarea terminaría después de $S+1$.
    - Se ubica la tarea extra en intervalo posterior al [W, W + 1], utilizando ese espacio para otra tarea. En este caso, la tarea extra acumularía latencia.
...
La clave de esta reducción está en que Vertex Cover consiste en cubrir todas las aristas mientras que Dominating Set consiste en cubrir al menos una arista de cada vértice, no todas (Para un grafo conexo. Lo que quiero destacar es la relación conjunto-subconjunto de aristas tomadas en cuenta para cada problema). Entonces, debemos buscar una forma en la que Dominating Set tome en cuenta todas las aristas.
...
2. Si el grafo original tiene un Vertex Cover de tamaño $k$, entonces los mismos vértices forman un Dominating Set de tamaño $k$ en el nuevo grafo. Si hay un dominant set de tamaño $k$ en el nuevo grafo, entonces los mismos vértices forman un Vertex Cover de tamaño $k$ en el grafo original, considerando que siempre habrá una solución que no utiliza los vértices "aristas" que siempre serían peor opción que los vértices originales que conectan.
...
Dado un problema de decisión $X$, su complemento $\neg X$ es otro problema de decisión cuya respuesta es la negación de la respuesta de $X$. Es decir, si la respuesta a $X$ es afirmativa, la respuesta a $\neg X$ es negativa, y viceversa.
...
Dada la doble implicancia, si $P = NP$, entonces $NP = co-NP$.

Por lo tanto, si $NP \neq co-NP$, entonces $P \neq NP$ ya que si $P = NP$, entonces $NP = co-NP$.
...
¿Conocen el juego en donde tienen jarras con capacidades distintas a k litros y deben llegar a un estado en donde una de las jarras tenga exactamente $n$ litros? Las operaciones posibles son: llenar una jarra, vaciar una jarra y trasvasar de una jarra a otra.

Es posible modelar el problema como un grafo implícito, en donde cada nodo es un estado de las jarras y cada arista es una operación que se puede realizar. Dado un estado $(x, y)$, los estados siguientes posibles son:
...
Este approach implica también una complejidad espacial exponencial, ya que crear el grafo implica tener todos los estados posibles en memoria. Y sin crear el grafo y solo simulando el comportamiento también, ya que se mantiene el historial de los estados visitados en memoria.
...
Entonces, BFS termina generando una cantidad exponencial de configuraciones para reducir hasta 1 a L, ramificando hacia todos los posibles estados siguientes en cada paso.
...
Para cada posible punto intermedio $C'$ vemos si podemos llegar de $C_1$ a $C'$ en hasta L/2 pasos y de $C'$ a $C_2$ en hasta L/2 pasos. Como sólo nos importa si existe o no un camino, no necesitamos guardar todos los caminos posibles, sino sólo si existe al menos uno.

Como se hacen hasta $log_2(L)$ = $n$ llamados recursivos para cada par de vértices y se hacen llamados recursivos dentro de cada uno de ellos, el algoritmo de Savitch tiene complejidad espacial $O(n^2)$; determinada por la profundiad de la recursión. Es decir, el problema de planificación es un problema en PSPACE.
...
Así como Cook y Levin demostraron que Satisfiability es NP-Completo, Stockmeyer y Meyer demostraron que QSAT es PSPACE-Completo. La definición de problemas PSPACE-Completos es análoga a la de problemas NP-Completos:
...
Si la cláusula se evalúa verdadera, gana el jugador 1, sino el jugador 2.

El problema es: ¿Existe una estrategia ganadora para el jugador 1?

Este problema es equivalente a Q3SAT: $\exists x_1 \forall x_2 \cdots \exists x_{n-2} \forall x_{n-1} \exists x_n \, \Phi(x_1, \dots, x_n)$, y es posible reducir QSAT a Q3SAT de forma que lo hicimos con SAT a 3-SAT.
